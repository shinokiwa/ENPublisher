...
describe('Flows/BatchSyncAll', function() {
	...
	describe('#Database', function() {
		it('データベースから全てのポストを取得して、guidとタイトルをSyncのノートリストに追加する。', function(done) {
			...
			test.step('Database', next)();
		});
	});
	describe('#Tag', function() {
		it('Evernoteからノートブック内のタグを取得して、guidと名称をSyncのタグリストに追加する。', function(done) {
			var next = function() {
				var list = sync.tagList.all();
				expect(list).to.length(10);
				expect(list[0]).to.have.property('key', 'TEST-TAG-GUID-1');
				expect(list[0]).to.have.property('body', 'TEST-TAG-1');
				done();
			};
			test.step('Tag', next)();
		});
	...
	describe('#flow', function () {
		it('フロー処理で各ステップを実行する。', function (done) {
			var next = function() {
				var list = sync.noteList.all();
				expect(list).to.length(303 + 32);
				expect(sync.USN).to.eql(46);
				expect(sync.lastSyncAll).to.be.instanceOf(Date);
				expect(sync.lastSync).to.be.instanceOf(Date);
				expect(sync._duration).to.eql(15 * 60);
